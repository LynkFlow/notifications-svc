...
import { findActiveEmailTemplate } from "../repositories/emailTemplateRepository.js";
import smtpTransport, { type MailTransport } from "./smtpTransport.js";
...
export async function sendTemplatedEmail(
  ...
  const template = await findActiveEmailTemplate(input.templateCode, input.locale);
  if (!template) {
    throw new AppError(
      404,
  ...
  try {
    ...
    throw new AppError(
      ...
      temporaryFailure ? "EMAIL_PROVIDER_UNAVAILABLE" : "EMAIL_PROVIDER_ERROR",
